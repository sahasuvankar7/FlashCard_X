      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-5 w-full justify-start px-20">
        {card.map((card,index) => (
          <Link href={`/cards/${card.id}`} key={card.id} className=" ">
            <div className={`border-[0.1px] border-gray-400 mb-1 h-full flex flex-col justify-between bg-slate-800 p-5 rounded-md  text-slate-300 transition-transform duration-300 hover:shadow-glow hover:scale-105 shadow-lg`}>
